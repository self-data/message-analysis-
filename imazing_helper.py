...
def _load_imazing_msg_file(
    ...
    # Message has this format (last row is missing from old versions):
            ...
            #  "What did you end up getting?.",
            #  "iMessage"]
    original_rows = comma.load(filepath, force_header=True)
    converted_rows = []
    for row in original_rows:
        ...
        converted_rows.append([
            "Received" if row["Type"] == "Incoming" else "Sent",
            dateparser.parse(row["Message Date"]).strftime("%b %d, %Y %H:%M:%S %p"),
            row["Sender Name"] if row["Type"] == "Incoming" else "me",
            row["Sender ID"] if row["Type"] == "Incoming" else "me",
            row["Text"],
            row["Type"],
        ])
    
    return _load_external_phoneview_msg_file(
...
def load_csv(
    ...
    # except:
    #     
    records = _load_imazing_msg_file(filepath=filepath)
...
def plot_texts(
    ...
    def aggregate_df(df):
        if count_or_volume == PlotStyle.COUNT:
            ...
            agg_df = df.resample(time_frequency).sum(numeric_only=True).drop(columns=["inbound"])
        else:
